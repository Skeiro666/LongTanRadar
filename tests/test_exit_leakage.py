...
def test_future_news_does_not_change_past_exit_score():
    ...
    s2 = eng.evaluate(
        ...
        position=pos,
        news={"direction": "positive", "score": 0.8},
        # "future" news would only appear if caller passes it — engine must only use provided as-of news
    )
    assert s1.get("exit_score") == s2.get("exit_score")
    # Changing news at same as_of CAN change score (that's current info) — but mutating after-the-fact
